...
class RenderParser {
     ...
     * 这三个属性是需要单独传入的
     */
    constructor(renderPath, methods, initState, referencedIdentifiers, usedState, customComponentNames, componentProperies, loopRefs, methodName) {
        ...
        this.propsSettingExpressions = new Set();
                            ...
                            if (!isFinalReturn) {
                                ...
                                if (callExpr.isCallExpression()) {
                                    ...
                                    if (t.isMemberExpression(callee) &&
                                        ...
                                        const hasIfAttr = jsxElementPath.node.openingElement.attributes.find(a => a.name.name === adapter_1.Adapter.if);
                ...
                if (expression.isStringLiteral()) {
                    ...
                    if (t.isMemberExpression(node.callee) &&
                        ...
                        if (adapter_1.isNewPropsSystem() &&
                            ...
                            !constant_1.DEFAULT_Component_SET.has(componentName.name)) {
                            return;
                        }
            ...
            JSXAttribute: (path) => {
                ...
                if (t.isJSXIdentifier(name) && jsxElementPath.isJSXElement()) {
                    ...
                    if (name.name === 'key' || isThirdPartyKey) {
                        ...
                        jsx_1.setJSXAttr(t.isJSXIdentifier(slot.openingElement.name, { name: 'block' }) ? slot.children[0] : slot, 'slot', t.stringLiteral(slotName));
        ...
        this.visitors = Object.assign({ MemberExpression: (path) => {
                const { object, property } = path.node;
        ...
        this.isEmptyProps = (attrs) => attrs.filter(a => {
            if (t.isJSXSpreadAttribute(a))
                return true;
            return ![adapter_1.Adapter.for, adapter_1.Adapter.forIndex, adapter_1.Adapter.forItem, 'id'].includes(a.name.name);
        }).length === 0;
        /**
         * jsxDeclarations,
         * renderScope,
        ...
        this.handleLoopComponents = () => {
            ...
            this.loopComponents.forEach((component, callee) => {
                ...
                if (this.loopRefs.has(component.node) || loopRefComponent) {
                    ...
                    const refDecl = utils_1.buildConstVariableDeclaration(refDeclName, env_1.isTestEnv ? callGetElementById : t.logicalExpression('&&', t.identifier('__scope'), t.logicalExpression('&&', t.identifier('__isRunloopRef'), callGetElementById)));
                    const callRef = t.callExpression(ref.fn, [t.identifier(refDeclName)]);
                    const callRefFunc = t.expressionStatement(env_1.isTestEnv ? callRef : t.logicalExpression('&&', t.identifier(refDeclName), callRef));
                    if (adapter_1.Adapter.type === "tt" /* tt */) {
                        body.push(t.expressionStatement(t.logicalExpression('&&', t.logicalExpression('&&', t.identifier('__scope'), t.identifier('__isRunloopRef')), t.callExpression(t.memberExpression(t.identifier('Taro'), t.identifier('handleLoopRef')), args.length === 2 ? [...args, t.nullLiteral(), ref.fn] : [...args, ref.fn]))));
                    }
                    else {
                    ...
                    blockStatementPath.traverse({
                        ...
                        JSXElement: path => {
                            ...
                            if (t.isJSXIdentifier(element.name) &&
                                ...
                                const variableName = `$compid__${utils_1.genCompid()}`;
                                const tpmlExprs = [];
                                ...
                                const compidTempDecl = utils_1.buildConstVariableDeclaration(variableName, t.callExpression(t.identifier(constant_1.GEN_COMP_ID), [t.templateLiteral([
                                        t.templateElement({ raw: '' }),
        ...
        if (t.isReturnStatement(parentNode)) {
            if (!isFinalReturn && !isIfStemInLoop) {
                ...
                if ((adapter_1.isNewPropsSystem()) && !this.finalReturnElement && process.env.NODE_ENV !== 'test') {
                    if (this.isDefaultRender) {
                        blockAttrs.push(t.jSXAttribute(t.jSXIdentifier(adapter_1.Adapter.if), t.jSXExpressionContainer(t.jSXIdentifier('$taroCompReady'))));
                    }
                }
    ...
    genPropsSettingExpression(properties, id) {
        return t.callExpression(t.memberExpression(t.identifier(constant_1.PROPS_MANAGER), t.identifier('set')), [t.objectExpression(properties), id]);
    }
    ...
    addIdToElement(jsxElementPath) {
        ...
        if (t.isJSXIdentifier(openingElement.name) &&
            !constant_1.DEFAULT_Component_SET.has(openingElement.name.name) &&
            /[A-Z]/.test(openingElement.name.name.charAt(0))) {
            if (this.isEmptyProps(openingElement.attributes)) {
                return;
            }
            const name = `$compid__${utils_1.genCompid()}`;
            const variableName = t.identifier(name);
            this.referencedIdentifiers.add(variableName);
            const idExpr = utils_1.buildConstVariableDeclaration(name, t.callExpression(t.identifier(constant_1.GEN_COMP_ID), [
    ...
    setCustomEvent() {
        ...
        if (body && t.isArrayExpression(body.value)) {
            ...
            let classProp = t.classProperty(t.identifier('$$events'), t.arrayExpression(usedEvents)); // babel 6 typing 没有 static
            classProp.static = true;
    ...
    setPendingState() {
        ...
        const propsStatement = [...this.propsSettingExpressions].map(expr => {
            if (typeof expr === 'function')
                return expr();
            return expr;
        });
        if (this.isDefaultRender) {
            this.renderPath.node.body.body = this.renderPath.node.body.body.concat(...propsStatement, buildAssignState(pendingState), t.returnStatement(t.memberExpression(t.thisExpression(), t.identifier('state'))));
        }
        else {
            ...
            //   }
            // }
            this.renderPath.node.body.body.push(...propsStatement, t.returnStatement(t.objectExpression(pendingState.properties.concat(usedState))));
