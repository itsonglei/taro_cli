...
class Transformer {
    constructor(path, sourcePath, componentProperies, sourceDir) {
        this.result = {
            template: '',
            components: [],
            componentProperies: []
        ...
        this.buildAnonyMousFunc = (jsxExpr, attr, expr) => {
            ...
            if (loopCallExpr) {
                ...
                classBody.push(t.classMethod('method', t.identifier(anonymousFuncName), [t.identifier(indexKey), t.identifier('e')], t.blockStatement([
    ...
    handleRefs() {
        ...
        if (_constructor && t.isClassMethod(_constructor)) {
            _constructor.body.body.push(t.expressionStatement(t.assignmentExpression('=', t.memberExpression(t.thisExpression(), t.identifier('$$refs')), t.arrayExpression(objExpr))));
        }
    }
    setComponentPath() {
        ...
        if (this.renderJSX.size) {
            this.renderJSX.forEach((method, methodName) => {
                this.result.template = this.result.template
                    + new render_1.RenderParser(method, this.methods, this.initState, this.refIdMap.get(method), this.usedState, this.customComponentNames, this.componentProperies, this.loopRefs, methodName).outputTemplate + '\n';
            });
        }
        else {
